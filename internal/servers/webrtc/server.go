...
type findSessionByUUIDReq struct {
	uuid uuid.UUID
	res  chan *session
}
...
type Server struct {
	...
	chFindSessionByUUID    chan findSessionByUUIDReq
...
func (s *Server) Initialize() error {
	...
	s.chFindSessionByUUID = make(chan findSessionByUUIDReq)
	...
	for {
		...
		case req := <-s.chNewSession:
			sx := &session{
				...
				pathManager:           s.PathManager,
				parent:                s,
				abrEnabled:            s.ABREnable && !req.publish, // ABR only for WHEP (reader) sessions
				abrSwitchCooldown:     s.ABRSwitchCooldown,
			}
			sx.initialize()
		...
		case req := <-s.chFindSessionByUUID:
			req.res <- s.findSessionByUUIDUnsafe(req.uuid)
		...
		case req := <-s.chDeleteSession:
			...
			delete(s.sessions, sx)
			delete(s.sessionsBySecret, sx.secret)
			sx.Log(logger.Info, "closing session after client DELETE")
			sx.Close()
		...
		case req := <-s.chAPISessionsGet:
			sx := s.findSessionByUUIDUnsafe(req.uuid)
			if sx == nil {
...
func (s *Server) findSessionByUUIDUnsafe(uuid uuid.UUID) *session {
	for sx := range s.sessions {
		if sx.uuid == uuid {
			return sx
		}
...
func (s *Server) findSessionByUUID(uuid uuid.UUID) *session {
	req := findSessionByUUIDReq{uuid: uuid, res: make(chan *session, 1)}
	select {
	case s.chFindSessionByUUID <- req:
		return <-req.res
	case <-s.ctx.Done():
		return nil
	}
