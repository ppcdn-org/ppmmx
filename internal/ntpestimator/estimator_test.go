...
func TestEstimator(t *testing.T) {
	prevTimeNow := timeNow
	t.Cleanup(func() { timeNow = prevTimeNow })
	...
	require.Equal(t, time.Date(2003, 11, 4, 23, 15, 11, 0, time.UTC), ntp)

	timeNow = func() time.Time { return time.Date(2003, 11, 4, 23, 15, 20, 0, time.UTC) }
	ntp = e.Estimate(6 * 90000)
	require.Equal(t, time.Date(2003, 11, 4, 23, 15, 12, 0, time.UTC), ntp)
}
