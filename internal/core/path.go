...
func (pa *path) doReloadConf(newConf *conf.Path) {
	...
	if len(pa.tencentForwarders) > 0 &&
		(newConf.ForwardTencent != oldConf.ForwardTencent ||
			newConf.ForwardTencentStreamKey != oldConf.ForwardTencentStreamKey) {
...
func (pa *path) setAvailable(
	...
	if pa.conf.ForwardTencent && pa.tencentWHIP.Enable {
		pa.startTencentForwarding()
	}
...
// IsOnline returns true when a publisher is connected.
func (pa *path) IsOnline() bool {
	return pa.source != nil
}

// StartOnDemandRecording starts recording regardless of conf.Record.
// Returns the file path pattern being used.
func (pa *path) StartOnDemandRecording(opts recording.Options) (string, error) {
	if pa.recorder != nil {
		return "", fmt.Errorf("already recording")
	}
	// Apply optional overrides
	if time.Duration(opts.SegmentDuration) > 0 {
		pa.conf.RecordSegmentDuration = conf.Duration(time.Duration(opts.SegmentDuration))
	}
	if opts.MaxPartSize > 0 {
		pa.conf.RecordMaxPartSize = conf.StringSize(opts.MaxPartSize)
	}
	pa.startRecording()
	return pa.conf.RecordPath, nil
}

// StopOnDemandRecording stops an on-demand recording.
// Does nothing if conf.Record is true (YML-configured recording).
func (pa *path) StopOnDemandRecording() (int64, int64, error) {
	if pa.recorder == nil {
		return 0, 0, fmt.Errorf("not recording")
	}
	pa.recorder.Close()
	pa.recorder = nil
	// TODO: track actual file size and duration
	return 0, 0, nil
}

// SplitRecording closes the current recording segment and starts a new one.
// If renameTo is non-empty, the segment that was just closed is renamed to
// it once fully written, and its path is returned (see
// recorder.Recorder.SplitSegment).
func (pa *path) SplitRecording(renameTo string) (string, error) {
	if pa.recorder == nil {
		return "", fmt.Errorf("not recording")
	}
	return pa.recorder.SplitSegment(renameTo)
}

// recordingOpts mirrors recording.Options without importing the package.

// startTencentForwarding starts one Forwarder per H264 video layer present
// in the stream (see forward.SplitLayers), each pushing that layer plus the
// shared audio to Tencent Cloud over WHIP. Mirrors startRecording().
func (pa *path) startTencentForwarding() {
	...
	for i, layer := range layers {
		f := &forward.Forwarder{
			Tencent:   pa.tencentWHIP,
			StreamKey: layer.StreamKey,
			Desc:      layer.Desc,
			Stream:    pa.stream,
			Parent:    pa,
		}
		f.Initialize()
		pa.tencentForwarders[i] = f
	}
}

func (pa *path) stopTencentForwarding() {
	for _, f := range pa.tencentForwarders {
		f.Close()
	}
...
func (pa *path) addReaderPost(req defs.PathAddReaderReq) {
	...
	// Request a keyframe from the publisher so the new viewer
	// doesn't have to wait for the next GOP interval.
	if pub, ok := pa.source.(defs.Publisher); ok {
		pub.RequestKeyFrame() //nolint:errcheck
	}
