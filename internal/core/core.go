...
import (
	...
	"github.com/bluenviron/mediamtx/internal/admin"
...
func New(args []string) (*Core, bool) {
	parser, err := kong.New(&cli,
		kong.Description("mmx "+string(version)+", "+runtime.GOOS+", "+getArch()),
		kong.UsageOnError(),
...
func (p *Core) createResources(initial bool) error {
	...
	if initial {
		p.Log(logger.Info, "mmx %s, %s, %s", string(version), runtime.GOOS, getArch())
	...
	if p.pathManager == nil {
		...
		p.pathManager = &pathManager{
			...
			tencentWHIP: forward.TencentConfig{
				Enable:    p.conf.TencentWHIPEnable,
				Endpoint:  p.conf.TencentWHIPEndpoint,
				Domain:    p.conf.TencentWHIPDomain,
				App:       p.conf.TencentWHIPApp,
	...
	if p.conf.WebRTC &&
		...
		if p.recMgr == nil && p.conf.MMXNodeRole == "NODE_ROLE_RECORDER" {
			...
			if err != nil {
				p.Log(logger.Warn, "recording store init failed: %v", err)
			} else {
	...
	// Admin backend
	if p.adminSrv == nil && p.conf.AdminAddress != "" {
		webRTCPort, err2 := portFromAddress(p.conf.WebRTCAddress)
		if err2 != nil {
		...
		if p.adminStore == nil {
			st, err2 := admin.OpenStore(filepath.Join("data", selfName+".db3"))
			if err2 != nil {
				// create data dir and retry
		...
		p.adminSrv = &admin.Server{
			Listen:           p.conf.AdminAddress,
			MMXBackend:       "http://127.0.0.1:" + webRTCPort,
			MMXAPI:           "http://127.0.0.1:" + apiPort,
			NodeName:         selfName,
			Store:            p.adminStore,
			Parent:           p,
...
func (p *Core) closeResources(newConf *conf.Conf, calledByAPI bool) {
	...
	closeWebRTCServer := newConf == nil ||
		...
		newConf.NetStorageS3AccessKey != p.conf.NetStorageS3AccessKey ||
	...
	closeAdmin := newConf == nil ||
		newConf.AdminAddress != p.conf.AdminAddress ||
