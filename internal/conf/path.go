...
type Path struct {
	...
	WHEPVideoTrackCount    int      `json:"whepVideoTrackCount"`
...
func (pconf *Path) setDefaults() {
	...
	// zero automatically negotiates up to the supported number of video tracks.
	pconf.WHEPVideoTrackCount = 0
	...
	pconf.RPICameraTextOverlay = "%Y-%m-%d %H:%M:%S - mmx"
...
func (pconf *Path) validate(
	...
	case strings.HasPrefix(pconf.Source, "whep://") ||
		...
		if pconf.WHEPVideoTrackCount < 0 || pconf.WHEPVideoTrackCount > 4 {
			return fmt.Errorf("'whepVideoTrackCount' must be between 0 and 4")
		}
