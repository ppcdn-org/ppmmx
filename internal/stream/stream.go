	...
	for origMedia, origFormats := range r.onDatas {
		...
		if sm == nil {
			s.Parent.Log(logger.Warn,
				"reader subscription references a media that doesn't belong to the stream; dropping it")
			continue
		}
		...
		for origFormat, onData := range origFormats {
			...
			if sf == nil {
				s.Parent.Log(logger.Warn,
					"reader subscription references a format that doesn't belong to the media; dropping it")
				continue
			}
			var filter OnDataFilterFunc
			if filters := r.onDataFilters[origMedia]; filters != nil {
				filter = filters[origFormat]
			}
			sf.onDatas[r] = readerOnData{filter: filter, data: onData}
		}
	}
